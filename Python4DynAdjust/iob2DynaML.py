...
def Stn_xml_str(Stn):
    ...
    xml_str=xml_str+'<NonGSNumber>' + Stn.SelectPoint + '</NonGSNumber>\n'
    xml_str=xml_str+'<SelectPoint>' + Stn.SelectRL + '</SelectPoint>\n'
    xml_str=xml_str+'<SelectRL>' + Stn.NonGSNumber + '</SelectRL>\n'
    xml_str=xml_str+'</AdditionalInfoStn-->\n'    
...
def Msr_xml_str(row,re_scale=1):
    ...
    Msr=DnaMeasurement()
    Msr.type = row[1]
    Msr.vscale=str(row[3]); Msr.pscale=str(row[3]); Msr.lscale=str(row[4]); Msr.hscale=str(row[5])
    ...
    ControlRec.Duration=datetime.datetime.strptime(row[25],"%Y-%m-%d %H:%M:%S")
    ControlRec.TimeStatus=row[26]; ControlRec.EphemerisType=row[27]
    ControlRec.AtReceiver=row[28]; ControlRec.ToReceiver=row[29]
    ControlRec.FrequencyMode=row[30]; ControlRec.SurveyTechnique=row[31]
    ControlRec.Solution=row[32]; ControlRec.EpochInterval=row[33]
    ControlRec.Class=row[34]; ControlRec.LevelDistance=row[35]
    ControlRec.InstrumentModel=row[36]; ControlRec.Derivation=row[37]
    ControlRec.NonGSNumber=row[38]
...
####         and the input contains error ellipse information for fixed marks   #####
...
if len(sys.argv) >1: adjustment_name = sys.argv[1].replace('.iob','.db')
...
cursor.execute("""CREATE TABLE IF NOT EXISTS OBSERVATIONS (
              ID integer, TYPE text, VSCALE double, PSCALE double, LSCALE double, HSCALE double, FIRST short text, SECOND short text,
              ...
              StartDateTime date, Duration time, TimeStatus text, EphemerisType text, AtReceiver text, ToReceiver text, FrequencyMode text,
...
lineCount=0; idcount=0; Tgtidcount=0; GNSSmarks=''
...
for linestr in f.readlines():
    ...
    if CurrentMsr.type=='D' and linestr[0:4] != ' DIR' and lineCount>1:
        cursor.execute("INSERT INTO OBSERVATIONS (ID, TYPE, VSCALE, PSCALE, LSCALE, HSCALE, FIRST, SECOND, VALUE, SDEV, TOTAL, INST_HEIGHT, TARG_HEIGHT, TARGETS, TARGETS_SDEV, DX, DY, DZ, VS_1_1, VS_1_2, VS_1_3, VS_2_1, VS_2_2, VS_3_1, StartDateTime, Duration, TimeStatus, EphemerisType, AtReceiver, ToReceiver, FrequencyMode, SurveyTechnique, Solution, EpochInterval, Class, LevelDistance, InstrumentModel, Derivation, NON_GS) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", 
                        [idcount, CurrentMsr.type, CurrentMsr.vscale, CurrentMsr.pscale, CurrentMsr.lscale, CurrentMsr.hscale, CurrentMsr.first, CurrentMsr.second, CurrentMsr.value,  CurrentMsr.stddev, CurrentMsr.total, CurrentMsr.instheight, CurrentMsr.targheight, CurrentMsr.targets, CurrentMsr.targetstddevs, CurrentMsr.dx, CurrentMsr.dy, CurrentMsr.dz, CurrentMsr.Vs[0,0], CurrentMsr.Vs[0,1], CurrentMsr.Vs[0,2], CurrentMsr.Vs[1,0], CurrentMsr.Vs[1,1], CurrentMsr.Vs[2,0],ControlRec.StartDateTime, ControlRec.Duration, ControlRec.TimeStatus, ControlRec.EphemerisType, ControlRec.AtReceiver, ControlRec.ToReceiver, ControlRec.FrequencyMode, ControlRec.SurveyTechnique, ControlRec.Solution, ControlRec.EpochInterval, ControlRec.Class, ControlRec.LevelDistance, ControlRec.InstrumentModel, ControlRec.Derivation, ControlRec.NonGSNumber])
        conn.commit()       
...
# eg.*CONTROL;ANG;20181213;TS16;C;MEAS
    ...
    if linestr[0:5] == ' ELEM' and CurrentMsr.type=='G' and lineCount==2:        
        ...
        CurrentMsr.Vs[1,2]=linestr[30:54].strip()
    ...
    while prevCnt!=cursor.execute("SELECT Count(STATIONS.STATION_NAME) AS CountOfSTATION_NAME FROM STATIONS;").fetchall():
        ...
        while prevCnt!=cursor.execute("SELECT Count(NETWORKS.STATION_NAME) AS CountOfSTATION_NAME FROM NETWORKS;").fetchall():
            ...
            sqlstring="INSERT INTO NETWORKS ( STATION_NAME, NETWORK ) \
                    ...
                    FROM NETWORKS) AS TMP LEFT JOIN NETWORKS ON TMP.STATION_NAME = NETWORKS.STATION_NAME \
        ...
        for row in qry:
            ...
            if row[15][:2] =='CC':
                if Stn.Constraint=='FFC':
                    ...
                    w_msrout.write(ErrEllip2Ycluster(CurrentStn,1000000))
        sqlstring="SELECT OBSERVATIONS.* \
                    FROM (NETWORKS INNER JOIN OBSERVATIONS ON NETWORKS.STATION_NAME = OBSERVATIONS.FIRST) \
                    WHERE (((NETWORKS.NETWORK)="+ str(network_num) + ")) \
                    ORDER BY OBSERVATIONS.ID;"
